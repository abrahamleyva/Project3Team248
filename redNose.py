...
mouthCascade = cv2.CascadeClassifier("hars/nose.xml")

# Loads our image with only the non-transparent sections visible
imgMustache = cv2.imread('filters/red.png', -1)
...
# Convert redNose image to BGR and save the original image size
...
while time.clock() < 17:
    ...
    for (x, y, w, h) in faces:
        ...
        for (nx, ny, nw, nh) in mouth:
            ...
            mustacheWidth = nw / 2
            mustacheHeight = mustacheWidth * 2 * origMustacheHeight / origMustacheWidth

            # Center the redNose on the bottom of the nose
            x1 = nx - (mustacheWidth / 4)
            x2 = nx + nw + (mustacheWidth / 4)
            y1 = ny + nh - mustacheHeight
            y2 = ny + nh + (mustacheHeight / 4)

            # Prevents the redNose from going outside of the face boundry
            if x1 < 0:
                x1 = 0
            if y1 < 0:
                y1 = 0
			...
			# Re-re-calculate the width and height of the redNose image
            mustacheWidth = x2 - x1
            mustacheHeight = y2 - y1

            # Re-size the original image and the masks to the redNose sizes
            # calcualted above
